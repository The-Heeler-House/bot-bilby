...
export default class CommandPreprocessor {
    ...
    async onTextCommandPreprocess(message: Message, services: Services) {
        ...
        try {
            ...
            await services.pager.sendError(error, "Trying to execute the " + commandName + " text command. See message " + message.url, services.state.state.pagedUsers, { message, args, services });
    ...
    async onSlashCommandPreprocess(interaction: BaseInteraction, services: Services) {
        ...
        try {
            ...
            await services.pager.sendError(error, "Trying to execute the " + interaction.commandName + " slash command.", services.state.state.pagedUsers, { interaction, services });
