...
export default class AddCharacterCommand extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .setDescription("Adds a character preset for Bot Bilby to switch to.")
    ...
    async execute(message: Message, args: string[], services: Services) {
        ...
        try {
            ...
            await message.reply(`Successfully created character preset \`${name}\`.`);
        } catch (error) {
            logger.error("Encountered error while trying to create character preset", name, "\n", error, "\n", error.stack);
            await message.reply(`That's awkward. I encountered an error while creating the character preset \`${name}\`. Please try again.`);
        }
    }
}