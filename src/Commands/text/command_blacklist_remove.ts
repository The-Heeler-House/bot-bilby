...
export default class CommandBlacklistCommand extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .addImplicitStringArgument(
            "command",
            "Command(s) to allow running. To add multiple command, separate them with a comma. Omit this argument to allow **all** commands.",
            false,
        )
    ...
    async execute(
        ...
        if (!foundData || foundData["command"].length == 0) {
            await message.reply("Error! User is not blacklisted yet.");
            return;
        }
        ...
        if (args["command"]) {
            for (let i of args["command"]
                ...
                if (!commandList.has(i)) {
                    await message.reply(
                        `Error! Invalid command to remove \`${i}\`.`,
                    );
                    return;
                }
                newList.delete(i);
        ...
        try {
            ...
            await message.reply(
                `Successfully removed command(s) from the blacklist for <@${args["user"]}>.`,
            );
        } catch (error) {
            logger.error(
                "Encountered error while trying to remove command from the blacklist for",
                args["user"],
                "\n",
                error,
                "\n",
            ...
            await message.reply(
                `Encountered error while trying to remove command from the blacklist for \`${args["user"]}\`. Please try again.`,
            );
        }
    }
}
