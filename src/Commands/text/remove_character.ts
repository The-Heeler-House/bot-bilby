...
export default class RemoveCharacterCommand extends TextCommand {
    ...
    async execute(
        ...
        try {
            ...
            await message.reply(`Removed character \`${args["character"]}\`.`);
        } catch (error) {
            logger.error(
                "Encountered error while trying to remove character",
                args["character"],
                "\n",
                error,
                "\n",
            ...
            await services.pager.sendError(
                error,
                "Trying to remove character " + args["character"],
                services.state.state.pagedUsers,
                { message, args, character },
            );
            await message.reply(
                `Error: Problem while removing the character \`${args["character"]}\`. Please try again.`,
            );
        }
    }
}
