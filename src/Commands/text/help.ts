...
function beautifyArgs(command: TextCommand) {
    return command.data.arguments
        .map(
            (argument) =>
                `${argument.required ? `\`<${argument.name}>\`` : `\`[${argument.name}]\``} (type: \`${TextCommandArgType[argument.type]}\`) - ${argument.description}`,
        )
        .join("\n");
}
...
export default class HelpCommand extends TextCommand {
    public data = new TextCommandBuilder()
        .setName("help")
        .setDescription(
            "Shows a list of all text commands available to you, or about one specific command if provided.",
        )
    ...
    async execute(
        ...
        if (searchTerm != undefined) {
            ...
            if (command == undefined) {
                await message.reply(
                    "Sorry, I couldn't find a command with that name. Please try again.",
                );
                return;
            }
            ...
            let embed = new EmbedBuilder()
                ...
                .setDescription(
                    `${!canExecuteCommand(command, message.member) ? ":warning: **Your permission level prevents you from running this command.**\n*See the bolded permission fields below for entries which apply to you.*\n\n" : ""}${command.data.description}`,
                )
                .setColor(16777215);
            ...
            if (command.data.arguments.length != 0) {
                embed.addFields({
                    name: "Arguments",
                    value: beautifyArgs(command),
                });
            }

            let rolePermissions = command.data.permissions.allowedRoles.map(
                (roleId) => {
                    return {
                        role: {
                            id: message.guild.roles.resolve(roleId).id,
                            name: message.guild.roles.resolve(roleId).name,
                        },
                        allowed: true,
                    };
                },
            ...
            rolePermissions.push(
                ...command.data.permissions.deniedRoles.map((roleId) => {
                    return {
                        role: {
                            id: message.guild.roles.resolve(roleId).id,
                            name: message.guild.roles.resolve(roleId).name,
            ...
            if (rolePermissions.length > 1) {
                embed.addFields({
                    ...
                    value: rolePermissions
                        .map(
                            (roles) =>
                                `${message.member.roles.cache.has(roles.role.id) ? `**${roles.role.name}**` : `${roles.role.name}`} ${roles.allowed ? "✅" : "🚫"}`,
                        )
                        .join("\n"),
                    inline: true,
                });
            ...
            if (command.data.permissions.allowedUsers.length != 0) {
                embed.addFields({
                    ...
                    value: command.data.permissions.allowedUsers
                        .map(
                            (user) =>
                                `<@${user}>${message.author.id == user ? " (You)" : ""} ✅`,
                        )
                        .join("\n"),
                    inline: true,
                });
            ...
            let paginatedMessage = new PageBuilder(
                ...
                Array.from(allTextCommands, (entry) => entry[1]) // Take all the text commands as an array
                    .filter((command) =>
                        canExecuteCommand(command, message.member),
                    ) // Filter out all commands the user cannot run themselves.
                    .sort()
                    .map(
                        (command) =>
                            ` - ${command.data.name}:\n   ${command.data.description}`,
                    ) // Format the help output as "name - description"
                    .join("\n") // Join into 1 string with each command as a newline...
                    .match(/(?=[\s\S])(?:.*\n?){1,20}/g), // ... then split every 20 newlines for each page.
