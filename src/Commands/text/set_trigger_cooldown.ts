...
export default class EditTriggerCooldownCommand extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .addImplicitStringArgument(
            "trigger_id",
            "The trigger ID to edit the cooldown length of.",
        )
        ...
        try {
            ...
            collector.on("collect", async (msg) => {
                if (isNaN(parseInt(msg.content))) {
                    await msg.reply(
                        "This did not resolve to a number. Please try again.",
                    );
                    return;
                }
                ...
                await msg.reply(
                    `Edited trigger \`${trigger}\`'s cooldown to ${parseInt(msg.content)} seconds.`,
                );
            });
            ...
            collector.on("end", (collected) => {
                if (collected.size == 0)
                    message.reply(
                        "The trigger cooldown edit has been cancelled due to 30 seconds passing without response being received.",
                    );
            });

            await message.reply(
                `Editing trigger \`${trigger}\` cooldown. Please send the amount of seconds you want the bot to wait before responding to a trigger again.`,
            );
        } catch (error) {
            logger.error(
                "Encountered error while trying to edit trigger cooldown",
                trigger,
                "\n",
                error,
                "\n",
            ...
            await services.pager.sendError(
                error,
                "Trying to edit trigger cooldown " + trigger,
                services.state.state.pagedUsers,
                { message, args, dbTrigger },
            );
            await message.reply(
                `That's awkward. I encountered an error while editing the \`${trigger}\` trigger cooldown. Please try again.`,
            );
        }
    }
}
