...
export default class SetCharacterCommand extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .setDescription(
            "Changes the username and avatar of Bot Bilby to predefined characters.",
        )
        .addImplicitStringArgument(
            "character",
            "The character preset to set to.",
        )
    ...
    async execute(
        ...
        try {
            ...
            ).setNickname(character.name);
            await message.client.user.setAvatar(
                Buffer.from(character.avatarImage.buffer),
            );

            await message.reply(
                `Applied character \`${args["character"]}\` to bot.`,
            );
        } catch (error) {
            logger.error(
                "Encountered error while trying to change avatar and username to character",
                args["character"],
                "\n",
                error,
                "\n",
            ...
            await services.pager.sendError(
                error,
                "Trying to change avatar and username to character " +
                    args["character"],
                services.state.state.pagedUsers,
                { message, args, character },
            );
            await message.reply(
                `Error! Problem while changing character to \`${args["character"]}\`. Please try again.`,
            );
        }
    }
}
