...
export default class CommandBlacklistCommand extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .setDescription(
            "Add a user to the command blacklist which would prevent them from running certain commands.",
        )
        .addUserMentionArgument("user", "The user to add to blacklist.")
        .addImplicitStringArgument(
            "command",
            "Command(s) to prevent them from running. To add multiple command, separate them with a comma. Omit this argument to block **all** commands.",
            false,
        )
    ...
    async execute(
        ...
        if (args["command"]) {
            for (let i of args["command"]
                .split(",")
                .map((v) => v.trim().toLowerCase())) {
                if (!commandList.has(i)) {
                    await message.reply(
                        `Error! No command found for \`${i}\`.`,
                    );
                    return;
                }
                newList.add(i);
        ...
        try {
            ...
            await message.reply(
                `Added command(s) to the blacklist for <@${args["user"]}>.`,
            );
        } catch (error) {
            logger.error(
                "Encountered error while trying to add command to blacklist for",
                args["user"],
                "\n",
                error,
                "\n",
            ...
            await message.reply(
                `Error! Problem while trying to add command to blacklist for \`${args["user"]}\`. Please try again.`,
            );
        }
    }
}
