...
export default class SetSpamDetect extends TextCommand {
    public data = new TextCommandBuilder()
        ...
        .addChannelMentionArgument(
            "channel",
            "Channel to setup or update detection",
        )
        .addImplicitStringArgument(
            "config",
            "JSON configuration for spam detection. If you don't understand what this arg is, you shouldn't use this command.",
        )
        .allowInDMs(false);
    ...
    async execute(
        ...
        // Schema validation for spam detection config
        ...
        for (let i of requiredKey) {
            if (
                ...
                await message.reply(
                    `Invalid value for key "${i}" in JSON object! Must be a non-negative number.`,
                );
                return;
            }
        }
