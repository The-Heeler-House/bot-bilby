...
export default class GuessCommand extends SlashCommand {
    ...
    async execute(
        ...
        async function playMultiplayer(
            ...
            async function askQuestion(
                ...
                await interaction.channel.send(
                    `> :question: **Question ${currNum}**: ${currentEpisode.description}`
                )
                ...
                answerMessage.on("end", (_) => {
                    if (!correct) {
                        interaction.channel.send(
                            `:hourglass: Time's up! The answer is ${currentEpisode.name}. No one gains a point!`
                        )
                        setTimeout(() => {
                            askQuestion(interaction)
                        }, 500)
            ...
            async function endGame(
                ...
                interaction.channel.send(
                    `:alarm_clock: **Game over!** The winner is: <@${scores[0][0]}>!`
                )
        ...
        async function playMultiplayerInit() {
            // start the game loop
            shuffle(episodes)
                ...
                switch (i.customId) {
                    ...
                    case "leave":
                        ...
                        if (location != -1) {
                            if (i.user.id != interaction.user.id) {
                                ...
                                await i.reply({
                                    content: `:x: The host cannot leave!`,
                                    ephemeral: true,
                                })
                            }
                        } else {
                    ...
                    case "start":
                        if (i.user.id == interaction.user.id) {
                            ...
                            await interaction.channel.send(
                                ":checkered_flag: Starting Game! To end the game, type `endgame` as the host."
                            )

                            await playMultiplayer(userID, onlyID)
                        } else {
                            await i.reply({
        ...
        async function playSingleplayer() {
            ...
            async function askQuestion(
                ...
                try {
                    ...
                    remainingLives--
                    await interaction.channel.send(
                        `:hourglass: **Time's up!** The answer is ${currentEpisode.name}. You have ${remainingLives} lives remaining.`
                    )
                    setTimeout(() => {
                        askQuestion(interaction)
                    }, 500)
            ...
            async function endGame(interaction: Interaction, score: number) {
                interaction.channel.send(
                    `:alarm_clock: **Game over!** Your score is ${score} episodes guessed.`
                )
                saveScore(`<@${interaction.user.id}>`, score)
                interaction.channel.send({ embeds: [await generateLeaderboard()] })
            }
        }
