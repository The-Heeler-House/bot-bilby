...
export default class KeepyUppyCommand extends SlashCommand {
    public data = new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub.setName("boop").setDescription("boop the balloon"),
        )
        .addSubcommand((sub) =>
            sub.setName("fan").setDescription("use the fan, if you're cheeky"),
        )
        .addSubcommand((sub) =>
            sub
                .setName("restart")
    ...
    async execute(
        ...
        // WARNING: The following logic is susceptible to race conditions if multiple users interact at the same time.
        // For true atomicity, consider using MongoDB transactions or $cond/$setOnInsert logic to ensure updates are not lost.
        // See: https://www.mongodb.com/docs/manual/core/transactions/ and https://www.mongodb.com/docs/manual/reference/operator/update/setOnInsert/

        const keepyUppyDbData = await keepyUppyCollection.findOne({
        ...
        const respond_boop = async () => {
            await keepyUppyCollection.updateOne(
                { channel: interaction.channelId },
                { $set: { balloon_state: "off_ground" } },
            );
            let msg = "";
            ...
            await interaction.reply({
                ...
                files: [
                    new AttachmentBuilder(
                        await sharp(
                            `${keepyUppyGameData}/${randomChoice<string>(keepyUppyMap.boop)}`,
                        )
        ...
        const respond_missed = async () => {
            await keepyUppyCollection.updateOne(
                { channel: interaction.channelId },
                { $set: { balloon_state: "almost_on_ground" } },
            );
            await interaction.reply({
                content:
                    "Oh no! You missed, and it's falling to the ground! Quick, boop it before someone use a fan on it.",
                files: [
                    new AttachmentBuilder(
                        await sharp(
                            `${keepyUppyGameData}/${randomChoice<string>(keepyUppyMap.on_air)}`,
                        )
                            .resize(160, 160, { fit: "inside" })
                            .toBuffer(),
                        { name: "missed.png" },
        ...
        const respond_land_on_cup = async () => {
            ...
            await interaction.reply({
                ...
                files: [
                    new AttachmentBuilder(
                        await sharp(
                            `${keepyUppyGameData}/${randomChoice<string>(keepyUppyMap.on_cup)}`,
                        )
                            .resize(160, 160, { fit: "inside" })
                            .toBuffer(),
                        { name: "on_cup.png" },
        ...
        const respond_blow = async () => {
            await keepyUppyCollection.updateOne(
                { channel: interaction.channelId },
                { $set: { balloon_state: "blown_by_fan" } },
            );
            await interaction.reply({
                ...
                files: [
                    new AttachmentBuilder(
                        await sharp(
                            `${keepyUppyGameData}/${randomChoice<string>(keepyUppyMap.fan)}`,
                        )
                            .resize(160, 160, { fit: "inside" })
                            .toBuffer(),
                        { name: "fan.png" },
        ...
        const respond_fail_blow = async () => {
            await interaction.reply({
                content:
                    "Woops, the fan just lost power! Balloon is in the same position!",
            });
        };
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "restart":
                if (keepyUppyDbData.balloon_state != "popped") {
                    await interaction.reply(
                        "You cannot restart an ongoing session!",
                    );
                } else {
                    await keepyUppyCollection.updateOne(
                        { channel: interaction.channelId },
            ...
            default:
                await interaction.reply(
                    "No action or invalid action specified! Use `/kp boop` to boop the balloon and `/kp fan` to use the fan.",
                );
                break;
        }
    }
