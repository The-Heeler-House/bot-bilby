...
export default class MuteroulettePlaySubCommand extends SlashSubCommand {
    ...
    async execute(interaction: ChatInputCommandInteraction<CacheType>, services: Services) {
        ...
        var user = await users.findOne({ user: interaction.user.id }) as WithId<MuterouletteUser>;
        ...
        if (user == null) {
            ...
            user = await users.findOne({ user: interaction.user.id }) as WithId<MuterouletteUser>;
        }
