...
async function processDirectives(
    ...
    if (variables[root] != undefined) {
        ...
        } else if (typeof variables[root] === "object") {
...
export async function processResponse(
    ...
    let variables = {
        ...
        group: function (argv: string[]) {
            ...
            if ((argv[0] ?? "") == "") return `[${captureGroups.join(", ")}]`;
            else return captureGroups[parseInt(argv[0])] ?? "undefined";
        },
        ...
        cur_time: function (argv: string[]) {
            ...
            return moment()
                .tz(argv[0] ?? defaultTz)
                .format(argv[1] ?? defaultFormat); // 2:00:00 PM
        },
        ...
        range: function (argv: string[]) {
            return (
                parseFloat(argv[0]) < parseFloat(argv[1]) &&
                parseFloat(argv[2]) > parseFloat(argv[1])
            ).toString();
        },
        ...
        math: function (argv: string[]) {
            switch (argv[0]) {
                ...
                case "//":
                    return Math.floor(
                        parseFloat(argv[1]) / parseFloat(argv[2]),
                    );
                case "**":
                    return parseFloat(argv[1]) ** parseFloat(argv[2]);
                case "%":
        ...
        rchoose: function (argv: string[]) {
            return argv[Math.floor(Math.random() * argv.length)];
        },
    };
    ...
    for await (const directive of directives) {
        ...
        let result = await processDirectives(
            directive.split(":"),
            message,
            trigger,
            variables,
        ...
        for (var i = currentIndex + 1; i < directives.length; i++) {
            ...
            console.log(
                `replacing {${directive}} in ${directives[i]} with ${result.text}`,
            );
            console.log(result.text);
            directives[i] = directives[i].replace(
                `{${directive}}`,
                result.text,
            );
        }
