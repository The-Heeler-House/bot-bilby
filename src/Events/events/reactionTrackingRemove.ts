    ...
    async execute(
        ...
        if (reaction.message.partial) {
            ...
            try {
                ...
                logger.error(
                    "Encountered an error while trying to fetch full message from a reaction remove event (for reaction tracking).\n",
                    error,
                    "\n",
                    error.stack,
                );
                await services.pager.sendError(
                    error,
                    "Trying to fetch full message from a reaction remove event.",
        ...
        const staffChatChannel = (await reaction.client.channels.fetch(
            channelIds.reactionLog,
        )) as TextChannel;
        ...
        await staffChatChannel.send(
            `${emote} **removed** by \`${member.displayName}\` (\`${member.id}\`): ${messageLink}${services.state.volatileState.trackedReactions.has(`${reaction.message.id}_${reaction.emoji.id || reaction.emoji.toString()}`) && Date.now() - services.state.volatileState.trackedReactions.get(`${reaction.message.id}_${reaction.emoji.id || reaction.emoji.toString()}`).timestamp < 3000 ? "\n:warning: **This was likely a ghost react. It was added and removed within 3 seconds.**" : ""}`,
        );
    }
}
