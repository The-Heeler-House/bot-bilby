...
export default class ModerationPingEvent extends BotEvent {
    ...
    async execute(client: Client, services: Services, message: Message) {
        ...
        if (
            [ChannelType.DM, ChannelType.GroupDM].includes(message.channel.type)
        )
            return; // Don't log DMs.
        if (message.author?.id == client.user.id) return; // Avoid media from self

        if (services.state.state.ignoredChannels.includes(message.channelId))
            return; // Don't log ignored channels.
        if (
            services.state.state.ignoredChannels.includes(
                (message.channel as GuildChannel).parentId,
            )
        )
            return; // Don't log children of ignored channels.
        if (
            (message.channel as GuildChannel).parent != null &&
            services.state.state.ignoredChannels.includes(
                (message.channel as GuildChannel).parent.parentId,
            )
        )
            return; // Don't log children of children of ignored channels... This is getting absurd.
        ...
        try {
            ...
            message.attachments.forEach(async (attachment) => {
                ...
                const extension =
                    attachmentSplitByDot[attachmentSplitByDot.length - 1];
                ...
                try {
                    let imageFromS3 = await services.s3.get(
                        "media",
                    ...
                    await logChannel.send({
                        files: [
                            new AttachmentBuilder(imageFromS3, {
                                name: attachment.name,
                            }),
                        ],
                        content: `File sent by <@${message.author.id}> deleted in <#${message.channel.id}>.`,
                    ...
                    if (err instanceof S3Error && err.code == "NoSuchKey") {
                        ...
                        logger.warning(
                            "Failed to talk to S3 host. Is the server up?",
                        );
                        await services.pager.sendPage(
                            "Warning: Failed to talk to S3 host. Is the server up?",
                        );
                        ...
                        logger.error(
                            "Encountered an error while trying to log deleted attachments.\n",
                            err,
                            "\n",
                            err.stack,
                        );
                        await services.pager.sendError(
                            err,
                            "Trying to log deleted attachments.",
