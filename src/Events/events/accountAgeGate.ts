    ...
    async execute(client: Client, services: Services, member: GuildMember) {
        ...
        if (accountAge < minAge) {
            await member
                ...
                .catch((error) => {
                    logger.error(
                        "Encountered an error while trying to inform user that they cannot join due to low account age. They most likely have DMs from server members off.\n",
                        error,
                    );
                });
            await member.kick(
                `Account age is less than ${services.state.state.joinGate} days`,
            );
            const channel = (await client.channels.fetch(
                channelIds.memberLog,
            )) as TextChannel;
            await channel.send(
                `Member KICKED by the account age gate.\nUser: <@${member.user.id}> [${member.user.username}] (${member.user.id})\nAccount age: ${(accountAge / dayToMiliseconds).toPrecision(2)} days < ${services.state.state.joinGate} days`,
            );
        }
    }
}
